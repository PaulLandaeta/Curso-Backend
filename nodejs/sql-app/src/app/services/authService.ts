...
export class AuthService {
  ...
  async login(loginDTO: LoginDTO): Promise<UserDto> {
    ...
    console.log(
      "🚀 ~ file: authService.ts:31 ~ AuthService ~ login ~ isPasswordValid:",
      isPasswordValid,
    );
    if (!isPasswordValid) {
    ...
    console.log(
      "🚀 ~ file: authService.ts:37 ~ AuthService ~ login ~ token:",
      token,
    );
    user.token = token;
    ...
    console.log(
      "🚀 ~ file: authService.ts:40 ~ AuthService ~ login ~ userUpdated:",
      userUpdated,
    );
